...
\subsubsection{Bolla Lista - demo}
\begin{lstlisting}
{
	title:{
		label: 'Title of list',
...
\end{lstlisting}
Vengono memorizzati il titolo della lista e un array di elementi. Per
ciascun elemento che compone la lista è presente la stringa da
mostrare, un indice necessario per la ricerca sul database e un
booleano che rappresenta se quell'elemento della lista è stato
spuntato o meno.
...
\end{lstlisting}
Vengono memorizzati un titolo per il sondaggio, un array di opzioni e
un array di votanti. Ciascuna opzione è composta da un identificativo,
una stringa da visualizzare e il numero di prefenze accordate a tale
opzione.
Vengono memorizzati inoltre gli utenti che hanno già espresso un voto
in tale sondaggio con lo scopo di impedire che si voti più di una volta.
...
Vengono memorizzati due valori: il numero casuale estratto e il
range in cui si può estrarre un eventuale nuovo numero.
